...
const CONTRACT_SEEDS = [
  { code: "CT-2024-001", name: "Plataforma de Pagamentos", client: "Nubank Serviços", segment: "Fintech", manager: "Ana Ribeiro", status: "Ativo", mrr: 148_000, costRatio: 0.62, startedMonthsAgo: 22 },
  { code: "CT-2024-014", name: "Modernização do Core Bancário", client: "Banco Aurora", segment: "Bancário", manager: "Carlos Menezes", status: "Ativo", mrr: 212_000, costRatio: 0.68, startedMonthsAgo: 20 },
  { code: "CT-2025-003", name: "Portal do Cooperado", client: "Cooperativa Vale Verde", segment: "Cooperativa", manager: "Marina Lopes", status: "Ativo", mrr: 86_000, costRatio: 0.58, startedMonthsAgo: 15 },
  { code: "CT-2025-008", name: "Motor Antifraude", client: "Seguros Atlântico", segment: "Seguros", manager: "Rafael Duarte", status: "Em risco", mrr: 64_000, costRatio: 1.09, startedMonthsAgo: 11 },
  { code: "CT-2025-021", name: "Data Lake Corporativo", client: "Grupo Meridiano", segment: "Varejo", manager: "Ana Ribeiro", status: "Ativo", mrr: 132_000, costRatio: 0.64, startedMonthsAgo: 9 },
  { code: "CT-2026-002", name: "App de Investimentos", client: "Vertex Capital", segment: "Investimentos", manager: "Juliana Prado", status: "Ativo", mrr: 97_000, costRatio: 0.6, startedMonthsAgo: 6 },
  { code: "CT-2024-006", name: "Integração de Adquirência", client: "PagueMais", segment: "Fintech", manager: "Carlos Menezes", status: "Encerrado", mrr: 74_000, costRatio: 0.71, startedMonthsAgo: 23, endedMonthsAgo: 7 },
  { code: "CT-2026-011", name: "Open Finance — Fase 2", client: "Banco Aurora", segment: "Bancário", manager: "Marina Lopes", status: "Em negociação", mrr: 118_000, costRatio: 0.6, startedMonthsAgo: 0 },
];
...
export const toMonthKey = (date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`;
...
function buildContracts() {
  return CONTRACT_SEEDS.map((seed, index) => {
    ...
    return {
      ...
      healthScore: Math.round(seed.status === "Em risco" ? 38 + random() * 18 : 72 + random() * 26),
    };
  });
}
...
export function summarize(rows) {
  ...
  return { revenue, expenses, profit, margin: revenue ? (profit / revenue) * 100 : 0 };
}

export function monthlySeries(rows, monthKeys) {
  const buckets = new Map(monthKeys.map((month) => [month, { monthKey: month, receita: 0, despesa: 0 }]));
...
export function categoryBreakdown(rows) {
  ...
  return [...totals.entries()]
    .map(([category, value]) => ({ category, value, share: grandTotal ? (value / grandTotal) * 100 : 0 }))
...
export function getOverview({ months, contractId = null }) {
  ...
  const previousRows = filterTransactions(transactions, scope).filter((row) => previousMonths.has(row.monthKey));
  ...
  const comparable = monthsCovered(currentRows) === months && monthsCovered(previousRows) === months;
  ...
  const deltas = comparable
    ? {
        ...
        margin: previousSummary.margin ? summary.margin - previousSummary.margin : null,
      }
  ...
  const contractRows = contracts
    .map((contract) => {
      ...
      return { contract, revenue: totals.revenue, expenses: totals.expenses, result: totals.profit, margin: totals.margin };
    })
  ...
  return {
    ...
    sparks: {
      ...
      margin: series.map((row) => ({ value: row.receita ? (row.lucro / row.receita) * 100 : 0 })),
    },
  };
}
